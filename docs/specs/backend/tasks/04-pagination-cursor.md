...
- Encoding: **Base64URL** (recomendado para query string) vs Base64 padrão  **Base64URL** 
- Versão do payload interno: fixar `v: 1` e política de breaking change sim
- TTL do cursor: sem expiração na v1 ou expirar após N horas? sem exp
- Hash de filtros no payload: algoritmo (ex.: SHA256 dos query params normalizados) qualquer
- `nextCursor` pode ser `null` ou apenas omitido na última página? (hoje: ambos mencionados) pode ser null
- Listagem sem `name` em produtos: Scan ou Query em índice auxiliar? (impacta payload do cursor — ver task 15) query em indice auxiliar, nuna realizar scan a não ser que não haja outra alternativa, quando isso ocorrer, comuncar
